...
#Line 624-625 is where we modify the Scope Tag so it only looks for one tag and not all of them

#Line 644-645 is where we put search of what Compliance policies we want to affect with the ScopeTag selected

#Line 706-707 is where we put wildcard to what Configuration PRofiles we want to affect with the ScopeTag selected
...
#This needs to be changed for DOD to "https://graph.microsoft.us"
$resourceAppIdURI = "https://graph.microsoft.com"

#This needs to be changed for DOD to "https://login.microsoftonline.us/$Tenant"
...
# This is where we update the Complicance Policy names so it only targets anything containing the specified text, no need for '*' as we are using contains operator
        ...
        if($CPs){
            ...
            write-host $CPs.displayName -foregroundcolor Cyan
